...
### Q1. Các container chính trong STL và khi nào dùng cai nào?
...
- VI: `vector` là mạng liên tục. Khi `size == capacity`, nó cấp phát buffer mọi (thường gấp đôi), move tất cả phần tử, và gìải phóng buffer cũ. Dieu này **invalidate tất cả pointer/iterator**. Dùng `reserve()` để tránh reallocation khi biết trước kích thước.
...
- VI: Ưu tiên `emplace_back` hon `push_back` — construct tại chỗ, tránh copy/move thừa.
...
### Q3. `map` vs `unordered_map` — khi nào dùng cai nào?
...
- VI: Iterator phần loại theo khả năng: **Input** (đọc 1 lần), **Output** (ghi 1 lần), **Forward** (đọc/ghi nhiều lan), **Bidirectional** (+lui), **Random Access** (+nhảy bất kỳ), **Contiguous** (C++20, memory liên tục). Algorithm yêu cầu iterator tối thìểu — VD `std::sort` cần Random Access.
...
- VI: `std::remove` Không xóa phần tử — nó dịch phần tử còn lại ve đầu và trả về iterator tới "rac" cũối. Phải gọi `erase()` để thực sự xóa. C++20 đơn gìản hoa với `std::erase` và `std::erase_if`.
...
- VI: `std::optional<T>` bieu dien giá trị **có thể có hoặc không** — thay thể nullable pointer và sentinel value. Truy cập optional rong bảng `*` là UB; `.value()` throw `bad_optional_access`.
...
- VI: `std::variant<A,B,C>` là **type-safe tagged union** — lưu một trong nhiều kiểu, biết kiểu nào đang active, và gọi destructor dùng. Khác với `union`, truy cập sai kiểu sẽ throw `bad_variant_access` thay vì UB.
...
- VI: `string_view` là **non-owning, không allocate** reference đến chuoi ky từ. Tránh copy khi truyen string vào hàm. Canh báo: không báo gio trả về `string_view` trỏ vào local `std::string` — sẽ dangle.
...
- VI: Sau `std::move`, object bi move o trang thai **hợp le nhưng không xác định** — an toàn để destroy hoặc gần lai, nhưng không đọc giá trị.
